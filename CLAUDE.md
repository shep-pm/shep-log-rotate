...
- `shep-client` comes by version from crates.io, and is the only path to shep-core: `shep_client::shep_core`, never a second direct dependency. The floor is 0.10.0, where the `DogConfig` derive became the `dog_config` attribute and `config_schema` stopped returning a `Result`. Both are compile-time: below it `src/config.rs` names an attribute that does not exist, above it the derive is gone. A dog announces the `PROTOCOL_VERSION` of the shep-core it was built against and the shepherd refuses a handshake below its own `MIN_SUPPORTED`; 0.10.0 announces 9 and accepts 8. The lockfile pins the newest published and is refreshed deliberately. `Cargo.toml` carries the older floors and why each still holds.
